...
class TopBar(BoxLayout):
    ...
    def add_right_button(self, icon, on_release, text='', is_red_state=False):
        """ Add icon based buttons to the right side of the top bar title """
        button = IconButton(icon=icon, label_text=text, on_release=on_release, is_red_state=is_red_state, pos_hint={'center_y': .5})
        self.ids.button_container_right.add_widget(button)

    def add_left_button(self, icon, on_release, text='', is_red_state=False):
