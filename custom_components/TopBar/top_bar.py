...
class TopBar(BoxLayout):
    ...
    def add_right_button(self, icon, on_release):
        """ Add icon based buttons to the right side of the top bar title """
        button = IconButton(icon=icon, on_release=on_release, pos_hint={'center_y': .5})
        self.ids.button_container_right.add_widget(button)

    def add_left_button(self, icon, on_release):
